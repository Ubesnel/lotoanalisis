...
class LotteryCentenaWeekdayMV(models.Model):
    ...
    def init(self):
        ...
        self.env.cr.execute("""
            ...
            SELECT lo.sorteo_id, lo.week_day, 'hundreds_id' AS field_type, ln.name AS centena, COUNT(*) AS total_salidas
            FROM lottery_output lo
            JOIN lottery_number ln ON ln.id = lo.hundreds_id
            GROUP BY lo.sorteo_id, lo.week_day, lo.hundreds_id, ln.name
            UNION ALL
            SELECT lo.sorteo_id, lo.week_day, 'fireball_id' AS field_type, ln.name AS centena, COUNT(*) AS total_salidas
            FROM lottery_output lo
            JOIN lottery_number ln ON ln.id = lo.fireball_id
            WHERE lo.fireball_id IS NOT NULL
            GROUP BY lo.sorteo_id, lo.week_day, lo.fireball_id, ln.name;
        """)

        self.env.cr.execute("""
            CREATE INDEX idx_centena_weekday_mv
            ON lottery_centena_weekday_mv (sorteo_id, week_day, field_type, total_salidas DESC);
        """)
